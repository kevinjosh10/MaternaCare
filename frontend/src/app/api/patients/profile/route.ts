  ...
  try {
    ...
    try {
      ...
      if (patientResult.rows.length > 0) {
        ...
        const docsResult = await db.query(
          "SELECT id, file_name, file_key, s3_uri, public_url, file_size, status, ocr_markdown, extracted_entities, created_at FROM medical_documents WHERE patient_id = $1 OR patient_id = $2 OR patient_id = $3 ORDER BY created_at DESC",
          [patient.id, patient.email || "", patient.full_name || ""]
        );
        documents = docsResult.rows;
      }
    } catch (dbErr) {
      console.warn("[DB] Patient fetch note:", dbErr);
    }
    ...
    if (!patient) {
      ...
      return NextResponse.json({
        ...
        profile: {
          id: id,
          fullName: id.includes("@") ? "Priya Sharma" : id,
          age: "27",
          phone: "+91 98765 43210",
          email: id.includes("@") ? id : "priya.sharma@example.com",
          bloodGroup: "O+",
          gestationalWeeks: "32",
          dueDate: "2026-11-20",
          gravidity: "G2",
